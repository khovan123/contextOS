...
describe("live agent leaderboard", () => {
  it("runs through a CLI adapter when an agent binary is available", () => {
    ...
    const originalCommand = process.env.CONTEXTOS_FAKE_AGENT_CMD;
    process.env.CONTEXTOS_FAKE_AGENT_CMD = `${process.execPath} ${fake} {prompt_file}`;
    try {
      ...
      if (originalCommand === undefined) {
        delete process.env.CONTEXTOS_FAKE_AGENT_CMD;
      } else {
        process.env.CONTEXTOS_FAKE_AGENT_CMD = originalCommand;
      }
