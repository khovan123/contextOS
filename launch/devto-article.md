...
ContextOS treats project rules as runtime context, not static text.
...
Without that evidence, ContextOS should not confidently suggest EAS just because the prompt says "deploy".
...
ContextOS evidence benchmark: 80%
```

This is not a claim that ContextOS beats Codex, Claude Code, Cursor, or Gemini in live runs.
...
ContextOS is my attempt to build the missing pre-flight layer for coding agents.
